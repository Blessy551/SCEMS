...
// 🔥 FIXED PUBLIC EVENTS (NO BROKEN JOIN)
  ...
  try {
    const [events] = await pool.query(`
      SELECT
        ...
        COALESCE(NULLIF(e.Instructions, ''), e.EventType, '') AS description,
        e.EventDate AS date,
        e.StartTime AS time,
        v.Name AS venue,
...
// ✅ PUBLISH EVENT (UNCHANGED — WORKING)
const publishEvent = async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT EventID, OrganizerID FROM Events WHERE EventID = ?`,
      [id]
    );
    ...
    if (req.user.role === 'Organiser' && event.OrganizerID !== req.user.userId) {
      return res.status(403).json({ success: false, message: 'You can publish only your own events.' });
    }
...
// ✅ CANCEL EVENT (UNCHANGED — WORKING)
const cancelEvent = async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT EventID, RequestID, OrganizerID, EventName FROM Events WHERE EventID = ?`,
      [id]
    );
